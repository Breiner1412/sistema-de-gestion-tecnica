...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if (auth()->user()->tieneRol(...\App\Models\User::ROLES_GESTION))
                        <x-nav-link :href="route('soportes.index')" :active="request()->routeIs('soportes.*')" wire:navigate>
                            {{ __('Soportes') }}
                        </x-nav-link>
                    @endif
                    ...
                    @endif
                    @if (auth()->user()->tieneRol('admin', 'gerente', 'tecnico_soporte'))
                        <x-nav-link :href="route('informes.index')" :active="request()->routeIs('informes.*')" wire:navigate>
                            {{ __('Informes') }}
                        </x-nav-link>
                    @endif
                    @if (auth()->user()->tieneRol('admin', 'gerente'))
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if (auth()->user()->tieneRol(...\App\Models\User::ROLES_GESTION))
                <x-responsive-nav-link :href="route('soportes.index')" :active="request()->routeIs('soportes.*')" wire:navigate>
                    {{ __('Soportes') }}
                </x-responsive-nav-link>
            @endif
            ...
            @endif
            @if (auth()->user()->tieneRol('admin', 'gerente', 'tecnico_soporte'))
                <x-responsive-nav-link :href="route('informes.index')" :active="request()->routeIs('informes.*')" wire:navigate>
                    {{ __('Informes') }}
                </x-responsive-nav-link>
            @endif
            @if (auth()->user()->tieneRol('admin', 'gerente'))
                <x-responsive-nav-link :href="route('inventario.index')" :active="request()->routeIs('inventario.*')" wire:navigate>
                    {{ __('Inventario') }}
                </x-responsive-nav-link>
            @endif
            @if (auth()->user()->esAdmin())
